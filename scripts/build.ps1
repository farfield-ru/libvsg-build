...
# Without this, find_package(vsgall <version> CONFIG REQUIRED) - the ordinary
# versioned form - fails, and fails misleadingly: CMake reports "could not find
# a configuration file compatible with requested version", which reads as
# "package not found". AnyNewerVersion semantics.
$VersionFile = @'
# vsgallConfigVersion.cmake - generated by farfield-ru/libvsg-build.
set(PACKAGE_VERSION "__VSG_VERSION__")
...
'@.Replace('__VSG_VERSION__', $VsgTag.TrimStart('v'))
Set-Content -Path (Join-Path $VsgallDir 'vsgallConfigVersion.cmake') -Value $VersionFile -NoNewline
...
if ($ShippedCMake.Count -ne $ExpectedCMake.Count -or
    (Compare-Object $ShippedCMake.Name $ExpectedCMake)) {
